...
\item {\bf Open access to data and code}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: The dataset is hosted on Hugging Face (\url{https://huggingface.co/datasets/jhq0709/MedVIGIL}) and the evaluation harness on GitHub (\url{https://github.com/hq0709/MedVIGIL}); both URLs are in the abstract footnote. The release contains the 300-case manifest, the deterministic probe-expansion script, the per-model baseline runners, the integrity validator, the LLM-judge for the open-ended variant, the metric aggregator, and Croissant 1.0 metadata. Cached per-probe letters for the audit are also released so that any score in Section~\ref{sec:experiments} can be recomputed from the per-probe trace.
    \item[] Guidelines:
    \begin{itemize}
...
\item {\bf Declaration of LLM usage}
    ...
    %this research?
    \item[] Answer: \answerNA{}
    \item[] Justification: For paper writing, LLMs were used only for grammar and wording polish. None of the released benchmark content was generated by an LLM: probe construction, ROI annotation, gold answers, the candidate-answer set, doctor-defined refusal letters, paraphrase / negation / specificity-drop / knowledge-only / hallucination-trap rewrites, and clinical risk tiers are all radiologist-authored, and the three image-side variants (ROI-masked, ROI-only, lr\_flip) are rendered programmatically with PIL from each case's radiologist-authored ROI bounding box. The released harness includes an optional judge-model script for scoring the auxiliary open-ended variant; this is a scoring tool, not part of the headline MCQ audit. The 16 audited VLMs are subjects of evaluation, not components of our pipeline.
    \item[] Guidelines:
    \begin{itemize}
