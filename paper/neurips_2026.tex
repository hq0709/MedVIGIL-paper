...
\section{Visual-Token Ablation Pilot: ROI Decay vs. Answer Trajectory}
\label{app:token_ablation}

The visual-decay sweep of Appendix~\ref{app:visual_decay_appendix} corrupts the entire image channel with isotropic Gaussian blur. The complementary, more diagnostic question is: how does each model's actual letter answer change as we remove the \emph{specific} visual tokens that carry the answer-relevant evidence? An evidence-grounded model should grow less confident and switch its letter (often to the doctor-defined refusal letter) as the ROI is progressively masked; an ungrounded model that answers from priors or non-ROI image cues will keep selecting the same letter at near-equal confidence regardless of how much of the ROI has been destroyed.

\textbf{Protocol.} We sample 13 stratified cases (4 per CRT tier) whose released ROI bounding box is well-defined. For step $k\in\{0,1,2,3\}$ we replace the top $k/3$ fraction of the ROI's pixels with mid-grey ($(128,128,128)$), so that step 0 is the unperturbed image and step 3 matches the \texttt{roi\_masked} probe variant of the main audit. For each (model, step, case) we run five self-consistency samples at temperature $0.7$ and record (i) the modal selected letter and (ii) a confidence proxy defined as the modal-letter share across the five samples. We track the \emph{letter-stability rate}: the fraction of cases where the step-$k$ modal letter equals the step-0 modal letter.

\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_visual_token_ablation.pdf}
  \caption{Visual-token ablation on a representative case (MVB-0031). \textbf{Left:} the four-step ROI mask sequence; the blue rectangle outlines the doctor-defined ROI and the red overlay marks the masked top-fraction. \textbf{Right:} aggregate confidence trajectory across the pilot ($n{=}13$ cases, three representative API models, five self-consistency samples per cell). Marker size at each step is proportional to the letter-stability rate; the bold letter above each marker is the model's modal letter on the example case.}
  \label{fig:visual_token_ablation}
\end{figure}

\textbf{Findings} (Figure~\ref{fig:visual_token_ablation}). Three real patterns emerge.
\textbf{(i) Gemini 3.1 Flash-Lite is the most ungrounded under ROI ablation.} On the example case (MVB-0031, ``Is there evidence of a right apical pneumothorax?'') Gemini selects letter \textsc{D} at every step from full image through 100\% ROI mask and stays $\geq 86.2\%$ confident throughout; in aggregate, $69.2\%$ of cases keep their step-0 letter at step 3. The model is committing to a non-refusal answer even after the answer-relevant evidence has been entirely destroyed---the precise failure mode the experiment is designed to surface, and the same behaviour reflected in Gemini's $13.5\%$ ROI-masked accuracy in Table~\ref{tab:probe_breakdown}.
\textbf{(ii) Claude Opus 4.7 changes its answer the most as the ROI is masked.} The example-case letter trajectory is \textsc{A}\,$\rightarrow$\,\textsc{B}\,$\rightarrow$\,\textsc{C}\,$\rightarrow$\,\textsc{A} and only $38.5\%$ of cases retain their step-0 letter at step 3, even though the model's confidence remains high ($\geq 92.3\%$). This is consistent with Claude Opus's lowest aggregate trap-\sfr\ ($22.0\%$, Table~\ref{tab:headline_results}): the model is sensitive enough to ROI degradation to switch its commitment, often to the explicit refusal option.
\textbf{(iii) GPT-4o sits in between.} Confidence stays moderate ($76.9\%$--$90.8\%$) and $61.5\%$ of cases keep the step-0 letter at step 3, indicating partial sensitivity to ROI removal but a non-trivial residual tendency to commit to a non-refusal answer (consistent with GPT-4o's $53.0\%$ aggregate trap-\sfr).
...
\textbf{Reproducibility.} The released \texttt{analysis/figure\_visual\_token\_ablation.py} reads \texttt{data/medvlm\_bench\_v1/visual\_token\_ablation.csv} (per-(model, step) modal-letter confidence and stability counts) plus \texttt{\_visual\_token\_raw.jsonl} (per-job letter trace, used for the example-case annotations) and recomputes the plot deterministically. The runner \texttt{scripts/bench\_visual\_token\_ablation.py} regenerates the variant images from each case's released ROI bbox and re-queries the API models; both files ship in the evaluation harness.
