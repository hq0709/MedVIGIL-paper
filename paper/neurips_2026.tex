...
To address this problem, we formalise what we call an \emph{evidence-contract} framework. Ordinary medical VQA implicitly assumes that visual evidence is available and premises are well-posed; when the answer-relevant image region is masked or the question embeds a premise the image contradicts, that contract is broken and the appropriate behaviour shifts from producing a better answer to recognising that the evidential basis has failed. Within this framework, we define \emph{silent failure under perturbed evidence} as the central measurable failure mode---a model receives a perturbed input and returns a non-refusal answer that accepts the perturbation (Figure~\ref{fig:motivation}). To operationalise this construct, we introduce \medvigil, a 300-case dataset-backed evaluation suite supervised end to end by four board-certified radiologists. Two attending radiologists (\textsc{R1}, \textsc{R2}) annotate every case in parallel---authoring the safety tier, gold answer, candidate-answer set, ROI box, laterality flag, and the full library of textual and image-side perturbations---and a senior consolidating radiologist (\textsc{P3}) adjudicates every \textsc{R1}--\textsc{R2} disagreement and finalises every released field; an independent fourth radiologist (\textsc{R4}), recruited from a different academic medical centre and blinded to construction, then answers every released probe to provide a construction-blind upper-bound human reference baseline. The benchmark spans eight probe families---five textual (hallucination-trap false premises, negation, specificity-drop, paraphrase, knowledge-only) and three visual (ROI-masked, ROI-only, laterality-flip)---together with 240 counterfactual triplets and clinician-finalised clinical risk tiers. It evaluates evidence-conditional trustworthiness across three separable dimensions---Capability, Safety, and Grounding---combined via the harmonic-mean \medvigil\ Composite Score (MCS, Section~\ref{sec:mcs}), a form specifically designed so that strong clean-input performance cannot mask unsafe failure or weak visual grounding.

We audit 16 vision-capable model configurations spanning five proprietary providers and two open-weight medical systems, together with two text-only language-prior baselines, generating up to 2{,}556 scored probes per model. The results reveal a consistent pattern of axis divergence. Claude Opus 4.7 leads overall on MCS ($69.2$) by balancing all three components; Gemini 3.1 Flash-Lite achieves the largest ROI contrast (VGR $+49.5$\,pp); GPT-4o drops to MCS $44.1$ because Safety is its bottleneck despite moderate clean-input accuracy; and the independent fourth-radiologist reference baseline reaches MCS $83.3$, $14.1$ points above the strongest audited model. Taken together, these findings confirm that accuracy, safe refusal, and visual grounding do not reduce to a single ranking, and that evidence-integrity evaluation is a necessary complement to ordinary diagnostic accuracy. The paper makes three main contributions:
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  \item We define deployment-oriented trustworthiness for medical VLMs as the conjunction of intact-input capability, evidence-conditional safe failure, and ROI-conditioned visual grounding, and formalise silent failure under perturbed evidence as its core measurable failure mode.
  \item We introduce \medvigil, a 300-case evaluation suite constructed end to end by four board-certified radiologists (three constructing, one independently evaluating), with eight probe families, 240 counterfactual triplets, clinician-finalised clinical risk tiers, and a seven-metric correctness-conditioned scoring suite (Overall Accuracy, \sfr, VGR, PR, NEG, SDR, LPA). The release ships with a Croissant 1.0 metadata file, RAI fields, and a paired open-ended variant.
  \item We propose the \medvigil\ Composite Score (MCS), a harmonic-mean composite that prevents any single axis from masking weakness in the others, and report a full 16-model audit with matched no-image ablations, a continuous visual-decay sweep, an ROI-mask visual-token ablation, and an independent fourth-radiologist reference baseline.
\end{enumerate}
...
Medical VQA datasets such as VQA-RAD, SLAKE, and PMC-VQA established image-question-answer evaluation as a core testbed for biomedical visual reasoning \cite{lau2018vqarad,liu2021slake,zhang2023pmcvqa}. More recent medical VLMs broaden this setting toward instruction following, few-shot multimodal learning, and radiology foundation-model behavior across heterogeneous imaging sources \cite{li2023llavamed,moor2023medflamingo,wu2023radfm}. This line of work is essential for measuring whether models can answer clinical visual questions at all. A concurrent audit of frontier VLMs on medical VQA further reveals severe grounding failures and clinically dangerous laterality confusion under intact-input scoring~\cite{chen2026auditfrontier}, corroborating the premise that intact-image accuracy alone hides visual-grounding pathologies. However, most evaluations retain an intact evidence contract: the image is assumed to contain diagnostically usable information, the question premise is assumed to be valid, and performance is summarized by answer accuracy or generation quality.

\subsection{Hallucination, Priors, and Robustness Address Adjacent Failures}

Several related literatures stress-test this intact-input assumption from different angles. Hallucination benchmarks such as CHAIR and POPE measure unsupported content under otherwise valid visual inputs~\cite{rohrbach2018chair,li2023pope}, and recent medical LVLM work extends this concern to clinically meaningful hallucination severity, mitigation strategies, and systematic failure-mode taxonomies~\cite{chen2024medihallv1,chang2025medheval,gu2024medvh}. HEAL-MedVQA probes whether medical LMMs rely on visual grounding by combining textual and visual perturbations with anatomical segmentation masks~\cite{nguyen2025healmedicine}; \medvigil\ extends this probe-level design to an evidence-contract framing with risk-weighted safety scoring and counterfactual triplets rather than mask-localisation accuracy. Language-prior work in VQA, including VQA v2 and causal VQA studies, shows that models can answer from question regularities rather than visual evidence \cite{goyal2017vqa,agarwal2020causalvqa}; VLind-Bench and related counterfactual-image evaluations demonstrate that almost all current LVLMs exhibit significant language-prior reliance~\cite{lee2025vlindbench}, a pattern consistent with \medvigil's text-only-answerability flag and LPA diagnostic. Robustness benchmarks study whether predictions remain stable under corruptions, rephrasings, composition shifts, or medical-imaging domain shift \cite{hendrycks2019imagenetc,shah2019cycleconsistency,gokhale2020vqalol,guan2022domain,finlayson2019adversarial}. These settings motivate \medvigil, but they do not fully capture its target: when the answer-relevant visual evidence is absent, masked, or contradicted by a false premise, preserving an answer can itself be unsafe.

\subsection{From Abstention to Evidence-Conditional Safe Failure}

Selective prediction and selective generation provide a decision-theoretic basis for abstaining under uncertainty \cite{geifman2017selective,ren2023selfevaluation,wen2024knowyourlimits}. Recent benchmarks make this concrete at scale: AbstentionBench shows that reasoning-tuned LLMs degrade abstention by 24\% on average across 20 datasets covering false premises, underspecification, and outdated information~\cite{kirichenko2025abstentionbench}; MM-AQA evaluates abstention along visual-modality-dependency and evidence-sufficiency axes in standalone VLMs and multi-agent systems~\cite{madhusudhan2026mmaqa}; VLM-DeflectionBench probes appropriate deflection (``I cannot answer'') under conflicting or insufficient evidence~\cite{moratelli2026deflectionbench}; and JBA introduces a thirteen-subtype taxonomy of false-premise questions for MLLMs~\cite{li2025jba}. Risk-stratified medical evaluation has been validated separately in CSEDB, which uses 30 weighted clinical-consequence criteria across 26 departments~\cite{wang2025csedb}, and clinical evaluation further requires that failure modes be weighted by possible harm, as emphasised in medical-answer evaluation frameworks such as Med-PaLM~\cite{singhal2023medpalm}. \medvigil\ connects these threads by making abstention conditional on visual-channel integrity for medical deployment: the desired behavior is not generic refusal, but doctor-adjudicated safe action for each probe (correcting a false premise, flagging an ROI-masked image as insufficient, or refusing under broken evidence), with risk-weighted scoring and clinician-authored hallucination traps. Appendix~\ref{app:benchmark_positioning} gives a compact feature matrix comparing \medvigil\ with these adjacent benchmark families.
...
\begin{figure}[!t]
  ...
  \caption{\medvigil\ benchmark architecture: \textbf{(A)} the doctor-authored evidence contract (image, question, five-option gold), \textbf{(B)} text-side and image-side contract-perturbation operators, and \textbf{(C)} the response manifold scored by the seven reported metrics aggregated into MCS (Eq.~\ref{eq:mcs}). Construction pipeline is detailed in Section~\ref{sec:pipeline}; evaluation pipeline in Section~\ref{sec:eval_pipeline}.}
  \label{fig:pipeline}
\end{figure}
...
\medvigil\ involves four board-certified radiologists end to end: three construct the released suite, and an independent fourth supplies the human reference baseline (Section~\ref{sec:experiments}, Appendix~\ref{app:clinician_pilot}). The three construction radiologists are two attending-level radiologists (\textsc{R1}, \textsc{R2}) who annotate every case in parallel and a senior consolidating radiologist (\textsc{P3}) who finalises every released field. The fourth radiologist (\textsc{R4}, the baseline rater) is recruited from a separate institution and takes no part in construction. Each of the four has five or more years of post-residency clinical experience at academic medical centres. The top half of Figure~\ref{fig:pipeline} traces the construction workflow.
...
\textbf{Stage 2 --- Dual annotation and clinician-authored perturbation library.} \textsc{R1} and \textsc{R2} then independently annotate each surviving case along five dimensions: CRT (L1--L5), gold answer, five-option candidate set including the explicit refusal option, answer rationale plus differential set, and ROI bounding box with laterality flag; per-case inter-annotator agreement is logged. They then jointly author every text-side perturbation---paraphrase (T-CF), negation, specificity-drop, knowledge-only rewrite, and two hallucination traps---under clinical-realism constraints (trap premises invent only conditions a clinician might mis-state in a referral note). The three image-side variants---ROI-masked, ROI-only, lr\_flip---are rendered programmatically by a deterministic PIL utility from the radiologist-authored ROI bbox and laterality flag (mid-grey fill on the rectangular ROI, mid-grey fill on the complement, and full-image mirror, respectively). Every text-side perturbation is handwritten by \textsc{R1} and \textsc{R2} and bears their explicit sign-off before consolidation; every image-side perturbation is sign-off-checked by \textsc{R1}/\textsc{R2} after rendering.
...
The audit covers 16 vision-capable model configurations across five proprietary providers and two open-weight medical-specific systems (full snapshot list in Appendix~\ref{app:reproducibility}, Table~\ref{tab:model_snapshots}), with up to 2{,}556 scored probes per model. Blind-input controls (two DeepSeek text-only baselines and matched no-image ablations for selected GPT/Claude/Gemini systems) are in Appendix~\ref{app:noimg_ablation}. The independent radiologist reference baseline (\textsc{R4}, Appendix~\ref{app:clinician_pilot}) is shown as the shaded \textsc{Doctors} row of Table~\ref{tab:headline_results} as a calibration anchor, not a leaderboard entry.
...
\FloatBarrier
\begin{table}[!htbp]
  \caption{Headline \medvigil\ results (\%; VGR in pp). Shaded \textsc{Doctors} row is the independent fourth radiologist \textsc{R4} (Appendix~\ref{app:clinician_pilot}), shown as a calibration anchor not as a leaderboard entry. PR is paraphrase accuracy on T-CF probes (Section~\ref{sec:per_family_metrics}). $\downarrow$ lower is safer, $\uparrow$ higher is better. $^{\dagger}$HuatuoGPT-V was trained on PubMedVision (overlaps VQA-RAD/SLAKE). Bootstrap CIs in Appendix~\ref{app:bootstrap_appendix}; weight/Grounding sensitivity in Appendix~\ref{app:mcs_sensitivity}.}
...
\textbf{Axes do not collapse.} Claude Opus 4.7 leads accuracy (Overall $76.5$\%); Gemini 3.1 Flash-Lite has the largest ROI contrast (VGR $+49.5$\,pp); GPT-5.5 leads knowledge-only accuracy (LPA $99.6$\%); aggregate \sfr\ is lowest for Claude Opus 4.7 ($22.0$\%) and Gemini 3.1 Flash-Lite ($22.3$\%). Accuracy alone does not imply trustworthy behaviour: Gemini 3 Flash reaches $71.9$\% Overall but $37.2$\% \sfr, and GPT-4o reaches $56.1$\% Overall but $53.0$\% \sfr.

\textbf{Human ceiling.} The independent radiologist baseline (\textsc{R4}) scores Original $95.3$\%, trap \sfr\ $5.8$\%, and MCS $83.3$, leaving a $14.1$-point composite headroom over the strongest audited model. \textsc{R4}'s raw VGR is only $+4.5$\,pp because \textsc{R4} also refuses correctly under ROI mask---the desired use-when-present, refuse-when-absent pattern, which is why VGR enters MCS through the clipped, ROI-masked-paired Grounding axis rather than as a standalone monotone score.

\textbf{MCS reorders by bottleneck.} Claude Opus 4.7 leads at $69.2$ on balance; Gemini 3.1 Flash-Lite ranks second at $66.0$ on Safety+Grounding strength; GPT-4o falls to $44.1$ because Safety is its bottleneck; Qwen3.5-9B remains lowest at $31.0$. Per-probe-kind, per-CRT, and per-source breakdowns are in Appendix~\ref{app:audit_breakdowns}; bootstrap rank stability and weight/Grounding sensitivity in Appendices~\ref{app:bootstrap_appendix},~\ref{app:mcs_sensitivity}; extended interpretation in Appendix~\ref{app:extended_discussion}.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_audit_summary.pdf}
  \caption{\medvigil\ audit summary. \textbf{Left:} MCS component decomposition---Capability, Safety ($100-\mathrm{SFR}_w$), Grounding, and harmonic-mean MCS, models ordered by MCS. \textbf{Right:} Risk-tier silent-failure heatmap; centre matrix is per-tier trap \sfr, left blue bar is original-probe accuracy, right red bar is $\mathrm{SFR}_w$ (Eq.~\ref{eq:sfr_w}), models sorted safest-first. Figure~\ref{fig:audit_summary} makes explicit where the human ceiling is most distant from frontier VLMs (ROI-masked refusal, L4--L5 traps).}
...
To localise where each model stops using the image and starts answering from language priors, we sweep an isotropic Gaussian blur ($\sigma\in\{0,2,4,8,16,32,64\}$\,px and no-image $\sigma{=}\infty$) over the 300 case images on four representative models, and define the language-takeover point $L^\star$ as the smallest $\sigma$ at which the residual visual contribution drops below 20\% of the clean-image contribution (full protocol in Appendix~\ref{app:visual_decay_appendix}).

\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_visual_decay.pdf}
  \caption{Visual information decay on the 300 image-required cases. Solid curves: MCQ accuracy as blur $\sigma$ grows from $0$ to $64$\,px and to no-image ($\sigma{=}\infty$, X marker). Dashed curves: text-answerable control. Shaded region right of $L^\star$ is the language-prior-dominated regime ($\geq 80\%$ of visual contribution lost). Per-tier breakdown and per-model table in Appendix~\ref{app:visual_decay_appendix}.}
  \label{fig:visual_decay}
\end{figure}

$L^\star$ separates the four representative models by a factor of four ($16\!\to\!64$\,px), and the curves do not track the headline MCS ordering---a model that uses vision robustly under degradation can still rank below one that uses vision well only on intact inputs. HuatuoGPT-V 7B has the highest no-image floor ($20.6$\%, four times that of GPT-4o), suggesting that domain-specific medical fine-tuning concentrates clinical knowledge in the language backbone rather than the visual aligner. Per-model curves and full per-tier results are in Appendix~\ref{app:visual_decay_appendix}.
...
\medvigil\ targets one component of deployment-oriented trustworthiness---unsafe non-refusal under controlled textual or visual evidence perturbations---and is complementary to clinical diagnostic accuracy, deployment readiness, and workflow-prevalence studies. Specific scoping limits apply. (i)~The visual axis covers ROI-masked, ROI-only, and a global laterality flip; the Gaussian-blur sweep (Section~\ref{sec:visual_decay}) is a four-model ablation, and full-image corruptions are out of scope. (ii)~V-CF is realised as a textual conditional anchor rather than a regenerated image, and the triplet-coherence axis TR-coh is released but not yet scored across all audited systems; we treat it as a deferred axis rather than retracted. (iii)~The severity-aware metrics (Critical-Findings Hallucination Rate, Severity Drift Magnitude) and patch-masking VFR are part of the broader \medvigil\ design but require severity scoring and counterfactual image generation not run here. (iv)~ROI bboxes are adjudicated benchmark regions for positioning binary masks, not pixel-level segmentation labels. (v)~The human reference baseline rests on full 300-case coverage by a single construction-blind rater (\textsc{R4}); a multi-rater extension with inter-rater $\kappa$ / Krippendorff's $\alpha$ and time-budgeted arm contrast is the natural follow-up. (vi)~Source images come from public or appropriately licensed corpora, with credentialed CXR shipped as reconstruction pointers (Appendix~\ref{app:documentation}); pretraining-overlap cannot be ruled out, and results should not be read as evidence on private hospital cohorts. (vii)~We do not yet ablate thinking mode, temperature, safety-instruction prompts, or medical-specialised model families. The refusal classifier and LPA proxy are auditable benchmark instruments, not clinical measurement tools.
