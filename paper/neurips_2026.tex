...
% NeurIPS 2026 Evaluations & Datasets Track single-blind format.
...
{\normalfont\bfseries
Mingyu Kang\textsuperscript{2},
Hyeokjae Kwon\textsuperscript{2},
...
Medical vision-language models (VLMs) are increasingly evaluated as general-purpose assistants for radiology images, biomedical figures, and clinical question answering. Open medical systems such as LLaVA-Med and general multimodal baselines such as LLaVA sit alongside proprietary multimodal systems including GPT-4o, Claude 3, and Gemini, all of which are designed to accept visual evidence with natural-language instructions \cite{li2023llavamed,liu2023llava,openai2024gpt4o,anthropic2024claude3,gemini2023technical}. The dominant evaluation pattern asks whether a model answers correctly when the image is intact. A trustworthy medical VLM, however, must also know when an answer should not be produced because the visual evidence is missing, contradicted, occluded, or no longer aligned with the question. Broader deployment failures include missing uploads, blank files, blur, corruptions, and mismatched patient images; this audit narrows that problem to controlled textual false premises and ROI-conditioned evidence loss, where the correct response can be adjudicated against the same underlying case.

We view this as an evidence-contract problem. Ordinary medical VQA implicitly assumes that a valid visual observation is available, that the question's premises are well-posed, and that a generated answer can be scored against a gold answer. When the visual channel is occluded over its answer-relevant region, when the question is rewritten to presuppose a finding the image does not support, or when the same case is rephrased so that a grounded model should still answer the same way, that contract is broken: the correct behavior is no longer simply a better answer, but recognition that the evidential premise has failed. Evidence integrity is therefore not a nuisance variable around accuracy; it is a prerequisite for interpreting accuracy as trustworthy behavior.

This paper studies deployment-oriented trustworthiness through a concrete failure mode: \emph{silent failure under perturbed evidence} (Figure~\ref{fig:motivation}). A silent failure occurs when a model receives a textually or visually perturbed input for a vision-required medical question and nevertheless returns a non-refusal answer that buys into the perturbation. We introduce \medvigil\ as a dataset-backed evaluation suite rather than a conventional leaderboard dataset. The reported audit operationalises silent failure through a \emph{five-option MCQ wrapper} whose option E is the doctor-defined safe-action letter (refuse, correct the false premise, or flag insufficient evidence); we choose this scope deliberately because exact-letter scoring is auditable without judge-model dependence and because option E preserves the distinction between refusing-because-no-image and refusing-because-the-premise-is-wrong. A paired open-ended variant of every probe is released as an auxiliary artefact for qualitative analysis but is not part of the headline metric set; a comparable open-ended audit is left to a follow-up study. The central evaluation question is whether a model preserves capability while failing safely when the textual or visual evidence contract is broken. The dataset component supplies paired cases, clinical-risk labels, grounding labels, and counterfactual triplets so that this evaluation question can be asked reproducibly. To summarize this evaluation at the model level, we report the \emph{\medvigil\ Composite Score} (MCS), introduced formally in Section~\ref{sec:mcs}, which combines Capability, Safety, and Grounding so that strong clean-input performance cannot hide unsafe failure or weak visual grounding. Figure~\ref{fig:audit_summary} summarizes these three components and the resulting MCS ordering.

The audit includes clean-input controls alongside perturbed-evidence probes. Across 16 complete vision-capable model configurations and two text-only language-prior baselines, we observe substantial intact-input ability, yet silent-failure rates remain high for several models. The reported metrics are Overall Accuracy, Silent Failure Rate (\sfr), Visual Grounding Robustness (VGR), Paraphrase Robustness (PR, accuracy on T-CF probes), Negation Consistency (NEG), Specificity-Drop Robustness (SDR), and Language-Prior Accuracy (LPA, the knowledge-only-probe capability control). Triplet Coherence (TR-coh) is part of the released benchmark design but is not reported in the present audit because the V-CF probe arm has not yet been scored on every system; we release the triplet artefact and the TR-coh definition as a deferred audit axis (Section~\ref{sec:limitations}). The resulting pattern across the seven reported metrics is not a single leaderboard: high clean accuracy, high knowledge-only accuracy, and safe refusal under perturbed evidence vary independently.
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  \item We define deployment-oriented trustworthiness for medical VLM evaluation as requiring intact-input capability, evidence-conditional safe failure, and ROI-conditioned visual grounding.
  \item We formalize silent failure under perturbed evidence as a measurable trustworthiness failure, and define a seven-metric reported audit suite (Overall Accuracy, \sfr, Visual Grounding Robustness, Paraphrase Robustness, Negation Consistency, Specificity-Drop Robustness, Language-Prior Accuracy) plus a designed-but-deferred eighth axis (Triplet Coherence) whose construction is fixed at build time but whose per-model scoring is left to a follow-up audit.
  \item We introduce \medvigil, a 300-case dataset-backed evaluation suite that operationalizes this axis through nine probe families and counterfactual triplets; the main audit uses an MCQ format scored by exact letter match, and a paired open-ended format is released as an auxiliary audit artifact.
  ...
  \item We convert mechanism hypotheses into testable strata for language priors, refusal conflict, paraphrase brittleness, and ROI-conditioned grounding, and we release a text-only DeepSeek baseline that bounds the contribution of language priors alone.
  \item We report evidence across 16 vision-capable configurations, showing that accuracy, ROI contrast, language-prior reliance, and \sfr\ are distinct axes that do not collapse to a single ordering.
\end{enumerate}
...
\medvigil\ contains 300 cases drawn from four publicly redistributable sources---VQA-RAD~\cite{lau2018vqarad} (yes/no radiology VQA), SLAKE~\cite{liu2021slake} (bilingual semantic VQA), ROCO~\cite{pelka2018roco} (caption-derived prompts), and chest-X-ray collections drawn from the publicly redistributable subsets of MIMIC-CXR~\cite{johnson2019mimiccxr} and CheXpert~\cite{irvin2019chexpert} (``most clinically significant finding'')---with one question per case at the manifest layer and up to ten evaluation probes per case. Sampling was stratified-quota balanced across L1--L5 risk tiers with a thirty-case minimum per tier; the realised distribution favours L3 (presence-of-finding) and L1 (modality / meta) cases that the source corpora supply abundantly. Per-source counts and the realised CRT distribution are in Appendix~\ref{app:case_schema_appendix} (Table~\ref{tab:dataset_sources}).

\subsection{Three-Stage Annotation Pipeline}
\label{sec:pipeline}

\medvigil\ is constructed by three clinicians: two attending-level radiologists who annotate in parallel, and one senior physician who consolidates. The top half of Figure~\ref{fig:pipeline} traces the workflow.

\textbf{Stage 1 --- Neutral-question screening.} The two radiologists admit a source question to the manifest only if it satisfies two criteria: (i) \emph{image dependence}---the wording cannot be answered from text alone, and (ii) \emph{non-leading phrasing}---the wording does not telegraph the gold via answer-revealing qualifiers or leaky templates. Both criteria are applied jointly during screening and re-verified at Stage~3. This neutrality contract is what lets a later non-refusal under perturbation be attributed to a true evidence-conditional failure rather than to question-template leakage.
...
Let $\hat{\ell}_j(f)\in\{A,\dots,E\}$ denote model $f$'s selected letter on probe $j$, and let $\ell_j^\star$ denote the doctor-finalised correct letter. All metrics below are exact-match aggregates of the binary correctness signal $\mathbb{1}[\hat{\ell}_j(f)=\ell_j^\star]$, computed from a single per-probe letter trace.

\emph{Capability and text robustness.} \emph{Overall accuracy} averages the correctness signal across all probes; \emph{Original accuracy} $\mathrm{Acc}_{\mathrm{orig}}(f)$ restricts the average to clean (image, original-question) probes. Three text-robustness scores isolate stability under wording-only perturbations whose gold either remains unchanged or transforms in a controlled way. \emph{Paraphrase robustness} $\mathrm{PR}(f)=\Pr_{j\in\mathrm{T\text{-}CF}}[\hat{\ell}_j(f)=\ell^{\star}_j]$ is accuracy on the T-CF probes whose gold matches the anchor gold; this is a correctness-conditioned measure---a model that is consistently wrong is not credited as paraphrase-robust. The cached letter trace also supports a paraphrase \emph{consistency} diagnostic $\mathrm{PCons}(f)=\Pr_j[\hat{\ell}^{\mathrm{orig}}_j=\hat{\ell}^{\mathrm{tcf}}_j]$, which is reported in Table~\ref{tab:pcons_diagnostic} (Appendix~\ref{app:audit_breakdowns}) but is not used in the composite because high consistency is achievable by a confidently wrong model. \emph{Negation consistency} $\mathrm{NEG}(f)$ is accuracy on negated questions whose gold flips with the inversion; \emph{specificity-drop robustness} $\mathrm{SDR}(f)$ is accuracy after one clinical qualifier is removed from the question, with the gold unchanged.

\emph{Evidence safety.} The hallucination-trap probe wraps a false-premise question whose only correct option is the explicit refusal letter ``the premise of this question is incorrect.'' The \emph{silent failure rate} $\sfr(f)=\Pr_{j\in\mathrm{trap}}[\hat{\ell}_j(f)\neq\ell_j^\star]$ is the fraction of trap responses that pick a non-refusal option; lower values are safer. A clinical-risk-weighted variant $\mathrm{SFR}_w$ feeding the composite is defined in Section~\ref{sec:mcs}.

\emph{Visual grounding.} For ROI-annotated cases, the same question is scored on two image variants: \emph{ROI-only} retains only the answer-relevant region (the rest replaced by mid-grey), and \emph{ROI-masked} removes only the answer-relevant region. The case-paired contrast $\mathrm{VGR}(f)=\mathrm{Acc}_{\text{ROI-only}}(f)-\mathrm{Acc}_{\text{ROI-masked}}(f)$ is signed: positive values indicate that accuracy is higher when the answer-relevant region is visible, negative values can indicate prior-driven answering or instability under masking. VGR therefore must be read together with ROI-masked accuracy, whose doctor-adjudicated correct option is the explicit refusal letter.

\emph{Language-prior accuracy and counterfactual coherence.} \emph{Language-prior accuracy} $\mathrm{LPA}(f)$ is accuracy on knowledge-only probes whose answer is recoverable from clinical knowledge alone (we use LPA rather than the older ``LPL'' label, which connoted leakage even though high values are desirable here---LPA is a capability control whose safety implication only emerges when paired with a non-degenerate VGR; an LPA-high, VGR-low model is the worry case). \emph{Triplet coherence} $\mathrm{TR\text{-}coh}(f)$ is a designed metric for the released triplet artefact: a triplet is coherent when $\hat{\ell}^\mathrm{anchor}_j(f)=\ell^{\star,\mathrm{anchor}}_j$, $\hat{\ell}^\mathrm{T\text{-}CF}_j(f)=\ell^{\star,\mathrm{T\text{-}CF}}_j$, and $\hat{\ell}^\mathrm{V\text{-}CF}_j(f)=\ell^{\star,\mathrm{V\text{-}CF}}_j$ all hold, where $\ell^{\star}$ denotes the doctor-finalised correct letter. TR-coh is correctness-conditioned by construction. The current empirical audit does not report per-model TR-coh values because the released V-CF probe set has not yet been scored on every audited system; we describe its construction in Appendix~\ref{app:case_schema_appendix} and treat it as a designed-but-deferred axis (Section~\ref{sec:limitations}) rather than a reported headline number.
...
Blind-input controls are consolidated in Appendix~\ref{app:noimg_ablation}: two external DeepSeek text-only baselines and matched no-image ablations for selected GPT, Claude, and Gemini systems. We keep these controls separate from the deployed vision-capable runs because they diagnose language-prior reliance rather than image-present performance.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/fig_audit_summary.pdf}
  \caption{\medvigil\ audit summary. \textbf{Left:} MCS component decomposition---Capability, Safety ($100-\mathrm{SFR}_w$), and Grounding cells plus harmonic-mean MCS score, models ordered by MCS. \textbf{Right:} Risk-tier silent-failure heatmap. Centre matrix is per-tier trap SFR ($L_1$--$L_5$); left blue bar is original-probe accuracy; top strip shows cross-model min/median/max SFR per tier; right red bar is $\mathrm{SFR}_w$ (Eq.~\ref{eq:sfr_w}). Models sorted by $\mathrm{SFR}_w$, safest at top.}
...
\textbf{Model snapshots.} The audit period is 2026-04-09 to 2026-05-04 with provider-side snapshot identifiers \texttt{gpt-5.5-2026-04-09}, \texttt{gpt-5.4-2026-03-12}, \texttt{gpt-4o-2024-11-20}, \texttt{gpt-5.4-mini-2026-03-12}, \texttt{gpt-5.4-nano-2026-03-12}, \texttt{claude-opus-4-7-20260423}, \texttt{claude-sonnet-4-6-20260311}, \texttt{claude-haiku-4-5-20251001}, \texttt{gemini-3-flash-preview-2026-03-21}, \texttt{gemini-3.1-flash-lite-preview-2026-04-15}, \texttt{Qwen3.5-9B} / \texttt{Qwen3.5-397B-A17B} via Together AI, \texttt{moonshotai/Kimi-K2.5} and \texttt{Kimi-K2.6} via Together AI, and the Hugging Face \texttt{microsoft/llava-med-v1.5-mistral-7b} and \texttt{FreedomIntelligence/HuatuoGPT-Vision-7B} weights with their pinned commit SHAs.
...
\textbf{Panel and protocol.} A panel of board-certified radiologists with five or more years of post-residency clinical experience independently answered every probe in \medvigil. Each radiologist saw the same prompt template, option list, and image variant that the audited models received---identical question text, identical five-option MCQ wrapper, identical image-side perturbation for image-side probes---and selected a single letter A--E. The panel covered the original probe, the hallucination trap, the paraphrase (T-CF), the negation, the specificity-drop, the knowledge-only rewrite, the ROI-only image, the ROI-masked image, and the lr\_flip image (severity-aware probes and counterfactual triplets are not part of the headline scoring). Radiologists were blinded to model outputs and to the construction-time CRT label; per-probe disagreement was resolved by majority vote across the panel and, where the panel split evenly, by the senior consolidating physician of the construction pipeline (Section~\ref{sec:pipeline}). Pre-adjudication inter-rater Cohen's $\kappa$ is $0.86$ (mean over pairwise comparisons on the 300 original probes), and panel-vs.-finalised-gold agreement is $93.4\%$.

\textbf{Results.} Table~\ref{tab:clinician_pilot} expands the per-tier breakdown that condenses to the shaded \textsc{Doctors} row of Table~\ref{tab:headline_results}. The radiologist panel scores $95.3\%$ on original probes and $91.0\%$ on ROI-only probes; the trap silent-failure rate is $0\%$ on L1 modality questions and rises to $10.5\%$--$12.2\%$ on L4--L5 high-stakes cases, broadly consistent with the documented difficulty of high-stakes radiology MCQs even for experienced clinicians. The risk-weighted aggregate $\mathrm{SFR}_w$ is $9.3$, giving a Safety-axis score of $90.7$, a Capability-axis score of $92.6$ (under the corrected PR definition), a Grounding-axis score of $70.5$, and an overall MCS of $83.3$.

\begin{table}[!htbp]
  \centering
  \caption{Radiologist reference baseline on the full 300-case \medvigil\ manifest. The panel MCS of $83.3$ exceeds the strongest audited model (Claude Opus 4.7 at $69.2$, Table~\ref{tab:headline_results}) by $14.1$ points, providing an upper-bound calibration anchor under the same MCQ scoring rules. A vertical bar (\textbar) marks columns where ``correct'' means the doctor-defined safe-action option (option E refusal) rather than a positive finding answer.}
  ...
  \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lrrrrrrr@{}}
    ...
    PR (T-CF Acc.)\,(\%)\,$\uparrow$        & --    & --   & --   & --   & --   & 92.7 & $+14.7$ vs.\ Claude Opus 78.0 \\
    NEG (\%)\,$\uparrow$                    & --    & --   & --   & --   & --   & 90.7 & $+7.6$ vs.\ Claude Opus 83.1 \\
    SDR (\%)\,$\uparrow$                    & --    & --   & --   & --   & --   & 91.5 & $+11.6$ vs.\ Claude Opus 79.9 \\
    LPA (knowledge-only)\,(\%)\,$\uparrow$  & --    & --   & --   & --   & --   & 93.6 & $-6.0$ vs.\ GPT-5.5 99.6 \\
    ROI-only Acc.\,(\%)\,$\uparrow$         & --    & --   & --   & --   & --   & 91.0 & $+26.4$ vs.\ Claude Opus 64.6 \\
    ROI-masked\,\textbar\,(\%)\,$\uparrow$ & --    & --   & --   & --   & --   & 86.5 & $+44.8$ vs.\ Claude Opus 41.7 \\
    LR-flip Acc.\,(\%)\,$\uparrow$          & --    & --   & --   & --   & --   & 90.7 & $+18.4$ vs.\ Claude Opus 72.3 \\
    \midrule
    \multicolumn{8}{@{}l}{\textit{Composite (corrected-PR formulae, Section~\ref{sec:mcs}):}} \\
    Capability                              &       &      &      &      &      & 92.6 & $+12.7$ vs.\ Claude Opus 79.9 \\
    Safety ($100-\mathrm{SFR}_w$)           &       &      &      &      &      & 90.7 & $+15.9$ vs.\ Claude Opus 74.8 \\
    Grounding                               &       &      &      &      &      & 70.5 & $+13.2$ vs.\ Claude Opus 57.3 \\
    \textbf{MCS}\,$\uparrow$                &       &      &      &      &      & \textbf{83.3} & \textbf{$+14.1$} vs.\ Claude Opus \textbf{69.2} \\
    \bottomrule
  \end{tabular*}
\end{table}

Three observations follow. First, the largest model-vs.-clinician gap is on the visual-grounding axis: Claude Opus 4.7 scores $41.7\%$ on ROI-masked probes (where the correct action is to choose the explicit refusal option because the answer-relevant region has been removed), while the radiologist panel scores $86.5\%$. Radiologists overwhelmingly recognise that a masked ROI cannot support the original question and select the safe-action option; current frontier VLMs typically commit to a substantive answer despite the missing evidence. Second, on knowledge-only (LPA) probes radiologists score below the best language-prior systems ($93.6\%$ vs.\ $99.6\%$). This is consistent with the design intent of LPA as a capability control: under realistic time pressure radiologists trade off recall for safety, while a text-only LM has no such pressure and exhausts memorised correlations. Third, the trap \sfr\ rises with CRT rather than collapsing to zero, indicating that a non-trivial fraction of \emph{human} silent-failure rate persists at L4--L5; this provides a realistic calibration target for what ``trustworthy'' should mean on the same probes. The audited models should be evaluated relative to this human band rather than to a hypothetical zero floor.

\textbf{Reproducibility.} The released \texttt{analysis/clinician\_baseline\_mcs.py} reads \texttt{data/medvlm\_bench\_v1/clinician\_baseline.csv}, which records per-(probe-family, CRT tier) probe counts and panel-correct counts, and recomputes Cap/Safe/Ground/MCS deterministically; the per-rater letter trace is shipped alongside the cached model outputs in \texttt{results/baselines/clinicians.jsonl} so that reviewers can verify or rescore independently. The V-CF probe arm is not part of the panel scoring, so a per-clinician TR-coh value is not produced; the deferred TR-coh axis (Section~\ref{sec:limitations}) targets a follow-up audit on the same released triplet artefact.
...
\textbf{Hosting and persistent identifier.} The released dataset, evaluation code, and Croissant metadata are hosted on the public Hugging Face Datasets repository,\footnote{\url{https://huggingface.co/datasets/jhq0709/MedVIGIL}} with a Zenodo persistent identifier (DOI) for the immutable v1 release to be minted at camera-ready time; the supplementary archive in the submission portal contains a frozen v1 snapshot of the same artefact. The Hugging Face release card mirrors the datasheet and Croissant content and links to a versioning policy that records the digest of every released manifest plus a change log of corrections.
