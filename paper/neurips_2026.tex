...
Medical vision--language models (VLMs) are moving from controlled benchmarks into clinical workflows---open medical systems such as LLaVA-Med~\cite{li2023llavamed} and HuatuoGPT-Vision~\cite{chen2024huatuogptvision} alongside proprietary GPT-4o~\cite{openai2024gpt4o}, Claude~\cite{anthropic2024claude3}, and Gemini~\cite{gemini2023technical}~\cite{moor2023medflamingo,wu2023radfm}. In high-stakes medical settings a trustworthy model must produce accurate responses \emph{and} behave safely when its inputs are unreliable, yet a growing body of evidence shows that current medical VLMs ground answers in surface terminology rather than fine-grained visual evidence.

A key driver is text-prior dominance. Frontier VLMs can rank top on standard medical imaging benchmarks without ever receiving an image, fabricating clinical detail from text alone~\cite{asadi2026mirageillusionvisualunderstanding}, and dramatically underperform young children on tasks requiring genuine visual perception~\cite{chen2026babyvision}. This is especially consequential in medicine, where biomedical text is rich enough that plausible-sounding answers can be assembled from question templates and terminology alone~\cite{goyal2017vqa,agarwal2020causalvqa}. Existing medical VLM evaluations, however, universally assume an intact evidence contract, and few systematically address the failure mode that follows---whether a medical VLM can recognise when the visual evidence has failed and refuse, rather than fabricate a fluent clinical answer~\cite{lau2018vqarad,liu2021slake,li2023pope,chen2024medihallv1,geifman2017selective,ren2023selfevaluation,singhal2023medpalm}.

We formalise this gap as an \emph{evidence-contract} framework: when the ROI is masked or the question carries a premise the image contradicts, the contract is broken and the appropriate behaviour shifts from producing a better answer to recognising that the evidential basis has failed. \emph{Silent failure under perturbed evidence}---a non-refusal answer that accepts the perturbation (Figure~\ref{fig:motivation})---is the central measurable failure mode. We operationalise the framework through \medvigil, a 300-case dataset-backed evaluation suite supervised end to end by four board-certified radiologists (R1, R2 parallel annotation; P3 senior consolidation; R4 construction-blind baseline; full pipeline in Section~\ref{sec:pipeline}). It spans eight probe families (five textual, three visual), 240 counterfactual triplets, and clinician-finalised risk tiers, and combines Capability, Safety, and Grounding via the harmonic-mean \medvigil\ Composite Score (MCS, Section~\ref{sec:mcs}) so that strong clean-input performance cannot mask unsafe failure or weak grounding.

We audit 16 vision-capable model configurations and two text-only language-prior baselines (up to $2{,}556$ scored probes per model) and observe consistent axis divergence: Claude Opus 4.7 leads MCS ($69.2$) by balance; Gemini 3.1 Flash-Lite has the largest ROI contrast (VGR $+49.5$\,pp); GPT-4o drops to MCS $44.1$ because Safety is its bottleneck; and \textsc{R4} reaches MCS $83.3$, $14.1$ points above the strongest audited model. Accuracy, safe refusal, and visual grounding do not reduce to a single ranking, and evidence-integrity evaluation is a necessary complement to diagnostic accuracy. The paper makes three main contributions:
...
We audit 16 vision-capable model configurations spanning five proprietary providers and two open-weight medical-specific systems (snapshot list, blind-input controls, and clinician baseline procedure in Appendices~\ref{app:reproducibility},~\ref{app:noimg_ablation},~\ref{app:clinician_pilot}); up to 2{,}556 probes are scored per model. The shaded \textsc{Doctors} row of Table~\ref{tab:headline_results} is the construction-blind \textsc{R4} baseline shown as a calibration anchor.
...
\textbf{Axes do not collapse, and accuracy alone is not trustworthiness.} Claude Opus 4.7 leads accuracy (Overall $76.5$\%); Gemini 3.1 Flash-Lite has the largest ROI contrast (VGR $+49.5$\,pp); GPT-5.5 leads LPA ($99.6$\%); aggregate \sfr\ is lowest on Claude Opus 4.7 ($22.0$\%) and Gemini 3.1 Flash-Lite ($22.3$\%). High Overall accuracy can coexist with high \sfr\ (Gemini 3 Flash $71.9$\% / $37.2$\%; GPT-4o $56.1$\% / $53.0$\%). The construction-blind radiologist baseline (\textsc{R4}) reaches MCS $83.3$, $14.1$ points above the strongest audited model; \textsc{R4}'s raw VGR is only $+4.5$\,pp because \textsc{R4} also refuses correctly under ROI mask, the use-when-present / refuse-when-absent pattern that motivates the clipped, ROI-masked-paired Grounding axis.

\textbf{MCS reorders by bottleneck.} Claude Opus 4.7 leads at $69.2$ on balance; Gemini 3.1 Flash-Lite is second at $66.0$ on Safety+Grounding; GPT-4o falls to $44.1$ on Safety; Qwen3.5-9B remains lowest at $31.0$. Per-probe-kind / per-CRT / per-source breakdowns are in Appendix~\ref{app:audit_breakdowns}; bootstrap and weight/Grounding sensitivity in Appendices~\ref{app:bootstrap_appendix},~\ref{app:mcs_sensitivity}; extended interpretation in Appendix~\ref{app:extended_discussion}.
...
\begin{figure}[!t]
  ...
  \caption{Visual information decay on the 300 image-required cases. Solid curves: MCQ accuracy as blur $\sigma$ grows from $0$ to $64$\,px and to no-image ($\sigma{=}\infty$, X marker). Dashed curves: text-answerable control. Shaded region right of $L^\star$ is the language-prior-dominated regime ($\geq 80\%$ of visual contribution lost). Full protocol, per-tier breakdown, and per-model table in Appendix~\ref{app:visual_decay_appendix}.}
  \label{fig:visual_decay}
\end{figure}

We sweep an isotropic Gaussian blur and define the language-takeover point $L^\star$ as the smallest $\sigma$ at which the residual visual contribution drops below $20$\% of the clean-image contribution (Figure~\ref{fig:visual_decay}). On four representative models, $L^\star$ varies by a factor of four ($16$--$64$\,px) and does not track the headline MCS ordering, and HuatuoGPT-V 7B has a no-image floor of $20.6$\%---four times GPT-4o's---suggesting that medical fine-tuning concentrates clinical knowledge in the language backbone rather than the visual aligner. The complementary ROI-mask token-ablation pilot at $T{=}0.7$ self-consistency is in Appendix~\ref{app:token_ablation}.
