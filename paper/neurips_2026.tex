...
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/motivation.png}
...
\begin{figure}[!t]
  ...
  \caption{\medvigil\ architecture: \textbf{(A)} doctor-authored evidence contract, \textbf{(B)} text-side and image-side perturbation operators, \textbf{(C)} response manifold scored by the seven reported metrics aggregated into MCS (Eq.~\ref{eq:mcs}). Construction in Section~\ref{sec:pipeline}, evaluation in Section~\ref{sec:eval_pipeline}.}
  \label{fig:pipeline}
\end{figure}
...
\medvigil\ targets unsafe non-refusal under controlled evidence perturbations and is complementary to diagnostic-accuracy and workflow-prevalence studies. (i)~The visual axis covers ROI-masked, ROI-only, and laterality-flip; the Gaussian-blur sweep is a four-model ablation, and full-image corruptions are out of scope. (ii)~V-CF is realised as a textual conditional anchor; triplet-coherence (TR-coh) is released but not yet audited across all systems---deferred, not retracted. (iii)~Severity-aware metrics (CFHR, SDM) and patch-masking VFR are designed but require severity scoring and counterfactual image generation not run here. (iv)~ROI bboxes are benchmark regions for binary masks, not segmentation labels. (v)~The human reference rests on full 300-case coverage by a single construction-blind rater \textsc{R4}; a multi-rater extension with inter-rater $\kappa$ / Krippendorff's $\alpha$ is the natural follow-up. (vi)~Sources are public or appropriately licensed (credentialed CXR as reconstruction pointers, Appendix~\ref{app:documentation}); pretraining-overlap cannot be ruled out, and results should not be read as evidence on private hospital cohorts. (vii)~We do not yet ablate thinking mode, temperature, safety-instruction prompts, or medical-specialised families. The refusal classifier and LPA proxy are benchmark instruments, not clinical measurement tools.
