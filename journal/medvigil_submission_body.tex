...
otherwise valid inputs, measurement hallucinations are corrected after the
fact in report generation~\cite{heiman2025factchexcker}, HEAL-MedVQA tests grounding through
...
\begin{figure*}[!tb]
  \centering
  \includegraphics[width=0.75\textwidth]{journal/figures/fig2.png}
...
the \texttt{plain} arm is bit-exact against the published rendering path and
that the annotated arms agree with it outside their annotation, a check on
pixels rather than on scores, since two different renderings can both score
50\%.
...
answer words. Over options of unequal length the likelihood favours the
string, not the content~\cite{holtzman2021surface}, so a refusal string can
pin the argmax to one option for every item. On a balanced corpus this scores 50\%, which an accuracy table cannot
...
\subsubsection{Statistical analysis} No hypothesis test is used to declare a capability. Intervals are 95\%
...
and volume-axis convention. Aria-25B-MoE was excluded because its
...
GPT-4o is the most visually anchored ($L^{\star}=64$), Claude Sonnet 4.6 has
the largest visual contribution (65.9\%$\rightarrow$9.1\%) but falls back at
$\sigma=32$, Qwen3.5-397B has the smallest contribution (40.5~pp) with the
earliest takeover, and HuatuoGPT-V 7B has the highest no-image floor
(20.6\%).
...
contain 25\%. Qwen2.5-VL-32B answers ``15'' on 296--300 of 300 items whether the outlines
enclose anatomy or nothing at all. Distance-estimation errors persist on synthetic backgrounds
(Fig.~\ref{fig:metric}b).
...
\begin{figure}[!tb]
  ...
  (c)~The modal answer and its share: preferences shift across backgrounds for some models, while Qwen2.5-VL-32B almost always answers “15 mm”. (d)~The same comparison as two numbers, as those
...
0.46--0.53 for every model, every interval containing 0.5. Image input provides no clear accuracy gain. Scale sharpens the reading of
...
present. Image input provides no clear accuracy gain on the 3D growth-matched subset.
...
channel is a property of the models and not of CT, which agrees with reports that general-domain VLMs fail elementary
geometric readouts and that metric spatial reasoning has required dedicated
training~\cite{rahmanzadehgervi2024blind}.
...
reaches a VLM, and route a failed check to a reader. These findings support checking the response channel before interpreting anatomical judgments.
