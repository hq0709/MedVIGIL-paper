...
\begin{figure}[!tb]
  \centering
  \includegraphics[width=\linewidth]{journal/figures/fig3.png}
  \caption{\textbf{One evaluation for both suites.} Standard VQA asks whether
  an answer is correct; this evaluation asks whether it remains supported as
  the evidence changes. (a)~Matched 2D and 3D cases with valid evidence and
  a well-posed premise. (b)~Controlled interventions: the image is full,
  ROI-masked, ROI-only or blank, the premise is paraphrased, negated or
  false, with a laterality flip in 2D and a matched growth pair in 3D.
  (c)~Each answer is matched against physician gold in 2D or computed gold
  in 3D and (d)~classed as a supported answer, a safe abstention or premise
  correction, expressed uncertainty, or a silent failure. (e)~Capability,
  safety and grounding feed the risk-weighted composite.}
  \label{fig:eval}
\end{figure}
...
because VGR and trap $\sfr$ vary independently. Under an isotropic Gaussian blur
sweep at $\sigma\in\{0,2,4,8,16,32,64\}$~px with a no-image control, the
...
GPT-4o is the most visually anchored ($L^{\star}=64$) and Qwen3.5-397B has
the smallest contribution (40.5~pp) with the earliest takeover.
...
34.7--56.3 points. In 3D the same subtraction on the growth-matched subset
...
regardless of what it computes, and seven of the thirteen sit at
$m\ge0.96$. Removing those seven and the three native systems that fail the
