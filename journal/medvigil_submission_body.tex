...
gap, one growth amount below the gap and one above it, so the two members of
a \emph{matched pair} have opposite answers by construction: labels are
...
correctness. A margin $m=2$~mm removes lesions already in contact and a
...
\begin{figure}[!tb]
  ...
  input of the sub-task decomposition (Fig.~\ref{fig:subtask}). (e)~The three
  \texttt{identified} panels as the model receives them, at one physical
  scale. Outlines are contours for display; the models receive 1-px pixel
  outlines.}
  \label{fig:render}
\end{figure}
...
\begin{figure*}[!t]
  \centering
  \includegraphics[width=0.75\textwidth]{journal/figures/fig3.png}
...
InternVL3-8B, Qwen3-VL-8B, Qwen2.5-VL-32B), and at 15.0--57.3\% once the
outlines and the legend are removed. They
...
computed from geometry, across twenty-nine model configurations. The 2D
finding survives the change of modality and worsens, the volumetric labels
cannot have been memorised, and the composite probe decomposes into steps
that fail selectively.
