...
\begin{figure}[!tb]
  ...
  evidence is checked.} (a)~2D. A chest radiograph is asked ``Is there a
  right apical pneumothorax?'' (gold ``yes''). In the matched case the apex
  region is masked and the question is edited to ask about a \emph{left}
  apical pneumothorax: the silent failure measures a pneumothorax in the
  masked apex on the wrong side, the safe response states that the region
  cannot be measured and corrects the premise. (b)~3D. An axial abdominal CT
  with the lesion outlined in red and the counterfactual growth boundary
  dashed in yellow. The surface gap is $d=10.2$~mm and the growth is 16~mm,
  so the computed gold is ``yes''; the silent failure answers ``no'' against
  the computed geometry. The CT is an illustrative case from the MSD
  hepatic-vessel task (CC~BY-SA~4.0) and the responses are illustrative.}
...
in tasks from report assistance to decision support~\cite{wu2023radfm}.
...
datasets~\cite{lau2018vqarad,liu2021slake} and the systems
trained on them~\cite{li2023llavamed,wu2023radfm} presume
...
This article removes that root cause where it can be removed. Given an
...
\begin{figure}[!tb]
  \centering
  \includegraphics[width=\linewidth]{journal/figures/fig2.png}
  \caption{\textbf{Two constructions, one evaluation.} (a)~2D,
  annotation-backed: 300 source cases from VQA-RAD, SLAKE, ROCO and chest
  radiograph collections; two radiologists screen and annotate each case
  (gold, A--E candidates, ROI, laterality, risk tier L1--L5); image and text
  perturbations are drafted as controlled variants of the same case; a
  senior radiologist finalises the gold keys, enforces the construction
  invariants and drops invalid probes. (b)~3D, simulator-backed: one
  Euclidean distance transform per lesion, computed with physical voxel
  spacing, gives the surface gap $d$ to each target; a matched pair states
  one growth below the gap and one above it ($d=10.2$~mm, $g^{-}=8$,
  $g^{+}=16$~mm); a provenance gate recomputes every label
  $y=\mathbf{1}[g\ge d]$ from the stored geometry. Both banks enter the
  shared evaluation of Section~\ref{sec:metrics}.}
  \label{fig:pipeline}
\end{figure}
...
reconstruction pointers. Each case carries the image, question, gold answer, a five-option set whose
...
perturbations are dropped, not patched. A fourth radiologist from a separate
...
in Section~\ref{sec:results_3d}. The \emph{identification control} holds the
probe fixed and varies the input through joint-visibility slices and
red/cyan outlines to \texttt{identified} (both, plus a 10~mm scale bar drawn
from the panel's own spacing, Fig.~\ref{fig:render}); an
\emph{input-richness} arm holds annotation at that level and shows three
planes, twenty-five contiguous axial slices, or the same planes magnified.
...
\begin{figure}[!tb]
  ...
  probe (colon, lesion 1, target liver; surface gap 3.97~mm). (a--d)~The
  axial panel under the four identification conditions. The published
  \texttt{plain} input takes the three centre slices, on which the lesion
  the question names has no voxel, as for 42.0\% of growth-matched probes;
  \texttt{bestslice} re-points the panels at slices showing both structures;
  \texttt{overlay} keeps the centre slices and adds the legend sentence, so
  the prompt describes an outline the image does not contain;
  \texttt{identified} combines joint-visibility slices, both outlines and a
  10~mm bar, and is byte-identical to the reader-study rendering and to the
  input of the sub-task decomposition (Fig.~\ref{fig:subtask}). (e)~The
  three \texttt{identified} panels as the model receives them, at one
  physical scale. Outlines are drawn as contours for display; the models
  receive 1-px pixel outlines.}
  \label{fig:render}
\end{figure}
...
the same synthetic outlines. The synthetic gaps follow the gap distribution
of the 300 real items and each gold is recomputed from the drawn masks, so
the three backgrounds differ only in what surrounds the outlines.
...
Twenty-nine configurations are audited. The 2D suite covers sixteen
models from OpenAI, Anthropic, Google, Qwen and Moonshot, the open medical
models LLaVA-Med-v1.5-7B and HuatuoGPT-Vision-7B, and two text-only
baselines; the volumetric suite covers thirteen, ten montage-fed
...
and volume-axis convention. InternVL3-78B and Llama-3.2-90B-Vision exceed two
80~GB cards and were not run, since quantising would change the claim under
test.
...
68.9\% of L3, L4 and L5 traps; Claude Opus 4.7, the safest model, retains
...
because VGR and trap $\sfr$ vary independently. Under an isotropic Gaussian
blur sweep at $\sigma\in\{0,2,4,8,16,32,64\}$~px with a no-image control,
the language-takeover point $L^{\star}$, the smallest $\sigma$ at which the
residual visual contribution falls below 20\% of the clean-image
contribution, varies by a factor of four across four representative models
(16--64~px) and does not track CS: GPT-4o is the most visually anchored
($L^{\star}=64$), Qwen3.5-397B has the smallest visual contribution
(40.5~pp) and the earliest takeover, and HuatuoGPT-V 7B has the highest
no-image floor (20.6\%, four times that of GPT-4o).
...
eighteen cells, four conditions for four models and two for
Qwen2.5-VL-72B, span 48.2--52.6\%. The two largest, Qwen3-VL-8B
...
(Section~\ref{sec:metrics}). Splitting \texttt{overlay} by whether the red
...
organs, chance 25\%). The four models with clean response channels localise
the red-outlined lesion to its organ at 67.7\%, 41.0\%, 81.7\% and 76.0\%
(Qwen2.5-VL-7B, InternVL3-8B, Qwen3-VL-8B, Qwen2.5-VL-32B), and at 57.3\%,
15.0\%, 49.7\% and 52.0\% once the outlines and the legend are removed. They
...
25.7\%, 26.3\%, 23.0\% and 28.3\% (Fig.~\ref{fig:subtask}), intervals
...
Qwen2.5-VL-32B 28.3\% throughout; all eight intervals contain 25.

\begin{figure}[!tb]
  \centering
  \includegraphics[width=\linewidth]{figs/fig_subtask.pdf}
...
\begin{figure}[!tb]
  ...
  disables the arithmetic they have.} (a)~One item under three backgrounds:
  synthetic outlines on uniform grey, the same outlines over real anatomy
  with its annotations erased, and the real lesion and target. (b)~Distance
  sub-task accuracy per model and background, volume-clustered 95\%
  intervals; synthetic gaps follow the real gap distribution and every gold
  is recomputed from the drawn masks. (c)~The modal answer and its share:
  the habit does not change with what the outlines enclose. (d)~The
  comparison the probe reduces to, posed as two numbers, as the same numbers
  inside the clinical sentence with no image, and as the published image.}
  \label{fig:metric}
\end{figure}
...
0.855, Qwen2.5-VL-32B 0.853), so the number is read and moves the score in
the right direction, and two rank it backwards (SmolVLM2-2.2B 0.39,
...
0.0\% of 2{,}262 probes [0.0, 0.0], and five others flip 0.0--1.2\%. Five
...
every model on every family. Under content-free calibration the same passes
separate: Qwen2.5-VL-7B declines 76.7\% of unsatisfiable premises at
$\sfrw=23.3\%$ and CS 49.1; M3D-LaMed declines 2.0\% at $\sfrw=97.3\%$ and
CS 7.1. On the cleanest trap family, calibrated $\sfr$ is
...
to 69.7 points, two text-only baselines score
...
$L^{\star}=16$--64~px. In 3D the same subtraction on
...
We evaluate evidence-conditional trustworthiness in two regimes, a
clinician-annotated 2D suite and a volumetric suite whose labels are
computed from geometry, across twenty-nine model configurations under one
...
and the composite probe decomposes into steps that fail selectively.
...
regardless of what it computes. Seven of the thirteen sit at $m\ge0.96$, so
their accuracy near 50\% follows from their answer distribution, and
InternVL3-14B's 50.1\% is confined to $[46.5, 53.5]$ before any reasoning is
considered. Removing those seven and the three native systems that fail the
...
wording is solved at 100\%; greedy decoding gives the same constants for
four of the five models. The clinical sentence costs the model the
arithmetic it otherwise performs. This was measurable only because the
labels were computed, which allowed the same items to be asked again in text
with their gold intact.

\subsection{What the Volumetric Regime Adds}

One control generalises beyond this corpus. The \emph{response-channel
control} was unnecessary in 2D, where every audited model answers seven
known-answer questions correctly, and is indispensable in 3D, where none of
the three native volumetric systems passes. A pinned output channel scores
50\% and cannot be told apart in an accuracy table from systems that fail
for other reasons. The control also anticipates the safety axis: every
failure is a failure of polarity. Among the four models measured on both
...
at 100\% fabricate on 43.7--73.2\%. A model that cannot produce a negative
...
grounding with intactness and VGR inverts instead of shrinking. A Ground
axis at its floor still contributes about 50 to a harmonic mean and reads as
partial competence, so in the volumetric regime the Ground axis should gate
the composite instead of averaging into it, and a system whose matched-pair
violation exceeds a threshold should have its CS reported as undefined. The
second instrument is the scoring rule: any sighted-versus-blind difference
scored against per-arm baselines inherits the artefact that a model's
content-free answer differs with and without an image, so the two arms need
a common baseline.
...
fine-tuning does not yet translate into safer behaviour. Two consequences
follow for use. An audit should report clean-input capability, the
risk-weighted silent-failure rate, the grounding decomposition,
language-prior controls and the response-channel gate as separate axes,
because they fail independently here and a composite hides which one failed.
...
reaches a VLM, and route a failed check to a reader instead of to a system
that answers fluently through it. A model that cannot say that a scan is not
a CT will not say that a lesion does not reach the aorta.
