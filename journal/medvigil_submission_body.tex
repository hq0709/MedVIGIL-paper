...
(Fig.~\ref{fig:render}); the eighteen cells, four conditions for four models
and two for Qwen2.5-VL-72B, span 48.2--52.6\%. The two largest, Qwen3-VL-8B
...
fine-tuning does not yet translate into safer behaviour. The corpus bounds
the reading in one direction: probe generation excludes every lesion within
2~mm of its target, so the cases in which management actually turns are
outside this audit by construction. An audit should
